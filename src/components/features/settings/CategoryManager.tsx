...
export const CategoryManager = () => {
 ...
 <span className="text-xs font-semibold px-2 py-1 bg-theme-elevated text-theme-tertiary rounded-full">
 {customCategories.length} / {MAX_CUSTOM_CATEGORIES}
 </span>
 </div>
 
 {customCategories.length === 0 ? (
 <p className="text-sm text-theme-tertiary italic p-4 bg-theme-surface rounded-xl border border-theme-border/50">
 No custom categories yet. Add one below!
 </p>
 ) : (
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
 {customCategories.map((cat) => (
 <div key={cat.id} className="flex items-center justify-between p-3 bg-theme-surface rounded-xl border border-theme-border/50 group">
 ...
 <span className="font-medium text-sm text-theme-primary">{cat.name}</span>
 </div>
 <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
 <button onClick={() => handleEdit(cat)} className="p-1.5 text-theme-tertiary hover:text-theme-primary hover:bg-theme-elevated rounded-md transition-colors">
 ...
 {!canAddMore && <p className="text-xs text-theme-tertiary mt-2">You have reached the maximum number of custom categories.</p>}
 </div>
 </div>

 {/* Preset Categories */}
 <div className="pt-6 border-t border-theme-border/30">
 <h4 className="font-medium text-theme-primary mb-4">Preset Categories (Default)</h4>
 <div className="flex flex-wrap gap-2">
 {presetCategories.map((cat) => (
 <div key={cat.id} className="flex items-center gap-2 px-3 py-1.5 bg-theme-elevated/50 rounded-full border border-theme-border/30">
 <span style={{ color: cat.color }} className="text-xs">
 {cat.icon ? <CategoryIcon iconName={cat.icon} size={14} /> : cat.emoji}
 </span>
 <span className="text-xs font-medium text-theme-tertiary">{cat.name}</span>
 </div>
 ))}
 </div>
 </div>
