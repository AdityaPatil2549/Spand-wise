...
export default function DashboardPage() {
  ...
  return (
    <div className="relative flex min-h-screen w-full bg-[#faf5ee] text-[#3a302a] font-body">
      ...
      <main className="flex-1 md:ml-64 flex flex-col px-[20px] md:px-[64px] py-[48px] overflow-x-hidden">
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-[24px] mb-16 lg:items-center">
          <div className="lg:col-span-7 flex flex-col gap-8 pr-0 lg:pr-12">
            <div className="space-y-4">
              <p className="text-[12px] tracking-[0.1em] font-bold text-[#c2652a] uppercase mb-2">Overview • {format(new Date(), 'MMMM yyyy')}</p>
              ...
              {isOverBudget ? (
                <>
                  ...
                  <TextEffect as="p" per="line" preset="blur" delay={0.2} className="text-[18px] text-[#605850] max-w-lg mt-8">
                ...
                <>
                  <h1 className="font-display text-[64px] md:text-[88px] leading-[0.9] font-medium text-[#3a302a] tracking-[-0.03em]">You are on <span className="text-[#10b981] italic">track.</span></h1>
                  <TextEffect as="p" per="line" preset="blur" delay={0.2} className="text-[18px] text-[#605850] max-w-lg mt-8">
              ...
              <AddExpenseMorph>
                <div className="mt-12 bg-[#c2652a] text-[#ffffff] px-8 py-4 rounded-full flex items-center gap-3 hover-elevate transition-all font-medium text-[20px] shadow-lg shadow-[#c2652a]/20 cursor-pointer">
          ...
          <div className="lg:col-span-5 w-full mt-12 lg:mt-0">
            <div className={`relative w-full rounded-[32px] overflow-hidden hover-elevate group ${isOverBudget ? 'ring-1 ring-[#c0392b]/30 shadow-[0_0_40px_rgba(192,57,43,0.15)]' : ''}`}>
              ...
              <div className="relative z-10 p-8 md:p-12 h-full flex flex-col justify-between min-h-[400px]">
                ...
                <div className="space-y-2 mt-12">
                  <p className="text-[16px] text-[#605850] font-medium">{isOverBudget ? 'Overage' : 'Current Balance'}</p>
                  <h2 className="font-display text-[56px] leading-none text-[#3a302a] tracking-tight flex items-center">
                    {isOverBudget ? '-' : ''}<AnimatedNumber value={Math.abs(balance)} />
                  </h2>
                </div>
                <div className="mt-12 space-y-4">
                  <div className="flex justify-between text-sm">
                    <span className="text-[#605850] flex items-center gap-1">Spent: <strong className="text-[#3a302a]"><AnimatedNumber value={totalSpent} /></strong></span>
                    <span className="text-[#605850] flex items-center gap-1">Budget: <strong className="text-[#3a302a]"><AnimatedNumber value={budgetAmount} /></strong></span>
                  </div>
                  <div className="w-full h-2 bg-[#e6e0d6] rounded-full overflow-hidden">
                    <div className="h-full bg-transparent w-full rounded-full relative">
                       <div className={`absolute top-0 left-0 h-full ${isOverBudget ? 'bg-[#c0392b]' : 'bg-[#c2652a]'}`} style={{ width: `${Math.min(100, (totalSpent / (budgetAmount || 1)) * 100)}%` }}></div>
        ...
        <section className="mt-8">
          ...
          {recentExpenses.length === 0 && !isExpensesLoading ? (
            <p className="text-[#605850]">No recent expenses.</p>
          ) : (
